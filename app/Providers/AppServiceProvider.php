           ...
           // Get most sold products for the charts
            $mostSoldProducts = Cache::remember('most_sold_products', 10, function () {
                return Order::join('exclusive_deals', 'orders.exclusive_deal_id', '=', 'exclusive_deals.id')
                    ...
                    ->whereIn('orders.status', ['pending','cancelled','completed', 'delivered']) // Only include these statuses
            ...
            $deductedQuantities = Cache::remember('deducted_quantities_by_generic_name', 10, function () {
                return Order::join('exclusive_deals', 'orders.exclusive_deal_id', '=', 'exclusive_deals.id')
                    ...
                    ->select(
                        'products.generic_name', // Get the generic name of the product
                        DB::raw('SUM(orders.quantity) as total_deducted') // Sum the quantities
                    )
                    ->groupBy('products.generic_name') // Group by generic name
                    ->orderBy('total_deducted', 'DESC') // Order by total deducted quantity
                    ->get();
            });

            $deductedLabels = $deductedQuantities->pluck('generic_name'); // Labels for the chart
            $deductedData = $deductedQuantities->pluck('total_deducted'); // Data for the chart
