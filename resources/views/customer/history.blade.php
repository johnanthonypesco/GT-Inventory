        ...
        @foreach ($groupedOrdersByDate as $dateName => $statuses)
            <div id="view-order-modal-{{ $dateName }}" class="fixed hidden bg-black/60 w-full h-full top-0 left-0 p-5 pt-20 z-50 backdrop-blur-sm">
                <div class="modal w-full lg:w-[80%] m-auto rounded-lg bg-white p-5 relative">
                    <span onclick="closeOrderModal('{{ $dateName }}')" class="text-xl font-semibold transition-all duration-150 text-black/80 hover:bg-gray-300 w-10 h-10 flex justify-center items-center rounded-full cursor-pointer absolute right-2 top-2 bg-gray-200 closemodal"><i class="fa-regular fa-xmark"></i></span>
