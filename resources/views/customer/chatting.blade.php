...
<body class="bg-gray-100 flex flex-col items-center justify-center min-h-screen">
    ...
    <div class="w-full max-w-lg bg-white shadow-md rounded-lg flex flex-col h-[600px]">
        <!-- Header -->
        ...
        <div id="chatBox" class="flex-1 p-4 overflow-y-auto space-y-2 bg-gray-50">
            @foreach ($conversations as $message)
                <div class="flex {{ $message->sender_id == auth()->id() ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-xs px-4 py-2 rounded-2xl shadow-md 
        ...
        <form action="{{ route('customer.chat.store') }}" method="POST" enctype="multipart/form-data" class="p-4 border-t bg-white flex items-center">
            ...
            <label for="fileInput" class="ml-2 cursor-pointer text-gray-600 hover:text-blue-600">📎</label>
